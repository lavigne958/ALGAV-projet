...
/**
 * soit 2 node* tel que n fils de father
...
int delete_node(node* n, node* father, int can_remove){
  ...
  node *gauche, *droit, *min;
  printf("entree delete_node\n");
  if(can_remove == -1){
    /*dans ce cas, on doit determiner si on peut supprimer le node*/
  ...
  if(!can_remove)
    ...
    fg_null = is_node_null(get_inf_node(n));
    if(!fg_null)
      gauche = get_inf_node(n);
    fd_null = is_node_null(get_supp_node(n));
    if(!fd_null)
	...
	if( !is_node_null(get_inf_node(gauche)))
	  set_inf_node(n,get_inf_node(gauche));
	if( !is_node_null(get_supp_node(gauche)))
	  set_supp_node(n,get_supp_node(gauche));
	if( !is_node_null(get_eq_node(gauche)))
	...
	if( !is_node_null(get_inf_node(droit)))
	  set_inf_node(n,get_inf_node(droit));
	if( !is_node_null(get_supp_node(droit)))
	  set_supp_node(n,get_supp_node(droit));
	if( !is_node_null(get_eq_node(droit)))
